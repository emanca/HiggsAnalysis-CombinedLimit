...
for chan in chans:
  print(chan)
  fShape[chan] = h5py.File('{}.hdf5'.format(chan), mode='r+')
  # print(list(fShape[chan].keys()))
...
for chan in chans:
  if chan in options.maskedChan:
    ...
    data_obs_chan = fShape[chan]["data_obs"][:]
    #exclude overflow/underflow bins
    nbinschan = int(data_obs_chan.shape[0])
    nbins += nbinschan
  ...
  for iproc,proc in enumerate(procs):
    ...
    for isyst,(name,syst) in enumerate(systsd.items()):
      ...
      if stype in ['lnN','lnNNoProfile','lnNNoConstraint']:
        ...
        systup_chan = fShape[chan]['{}_{}Up'.format(proc,name)][:]
        ...
        systdown_chan = fShape[chan]['{}_{}Down'.format(proc,name)][:]
